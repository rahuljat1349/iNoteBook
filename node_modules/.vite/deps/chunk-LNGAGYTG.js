...
function useTransitionTrigger(requestEnter) {
  const [state, dispatch] = React44.useReducer(transitionStateReducer, {
    elementExited: !requestEnter,
    inProgress: false
  });
  const hasTransition = React44.useRef(false);
  const handleEntering = React44.useCallback(() => {
    dispatch("entering");
  }, []);
  const handleEntered = React44.useCallback(() => {
    dispatch("entered");
  }, []);
  const handleExiting = React44.useCallback(() => {
    dispatch("exiting");
  }, []);
...
function useSnackbar(parameters = {}) {
  ...
  const timerAutoHide = React52.useRef();
  React52.useEffect(() => {
    if (!open) {
      return void 0;
    }
...
var TextareaAutosize = React69.forwardRef(function TextareaAutosize2(props, forwardedRef) {
  ...
  const renders = React69.useRef(0);
  const [state, setState] = React69.useState({
    outerHeightStyle: 0
  });
  const getUpdatedState = React69.useCallback(() => {
...
function CssTransition(props) {
  ...
  const [isEntering, setIsEntering] = React71.useState(!requestedEnter);
  React71.useEffect(() => {
    if (requestedEnter) {
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-LNGAGYTG.js.map
